...
contract ZeroCouponBond is Ownable {
	...
	uint256 totalDebt;
	uint256 parDecimals;
	uint256 bondsNumber;	// How many bonds to issue
	uint256 cap;
	uint256 parValue;		// value of bond at maturity - face value.
	uint256 couponRate;		// discount rate that will be payed (interest)
	uint256 term;			// how long the bond last for
	uint256 timesToRedeem;
	uint256 loopLimit;		// stay under "gas" limit for actions in pay out.
	uint256 nonce = 0;

	uint256 couponThreshold = 0;

	ERC20 token;		// Use an ERC-20 as the "holder" of the tokens.

	event MintedBond(address buyer, uint256 bondAmount);
		// emit MintedBond(buyer, _bondsAmount);
	event RedeemedCoupons(address sender, uint256[] bonds);
		// emit RedeemedCoupons(msg.sender, _bonds);
	event Transferred(address sender, address receiver, uint256[] bonds);
	...
	*/
	...
	/**
	* @notice Mint bonds to a new buyer - this is the bond "issue"
	...
	*/
	function redeemCoupons(uint256[] memory _bonds) public {
		...
		for (uint256 i = 0; i < _bonds.length; i++) {
			...
			if (bonds[_bonds[i]] != msg.sender || couponsRedeemed[_bonds[i]] == timesToRedeem) {
				continue;
			}
			...
			if (lastThresholdRedeemed.add(couponThreshold) >= maturities[_bonds[i]] ||
				block.timestamp < lastThresholdRedeemed.add(couponThreshold)) {
				continue;
			}
