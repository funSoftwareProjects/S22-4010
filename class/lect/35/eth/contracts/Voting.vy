...
# Setup global variables / Constructor - Assume that there are a max of 4
# things to vote for.
@external
def __init__(_proposalNames: bytes32[4]):
...
# Give a `voter` the right to vote on this ballot.  This may only be called by the `chairperson`.
@external
def giveRightToVote(voter: address):
	assert msg.sender == self.chairperson # Throws if the sender is not the chairperson.
	assert not self.voters[voter].voted   # Throws if the voter has already voted.
	assert self.voters[voter].weight == 0 # Throws if the voter's voting weight isn't 0.
...
def delegate(to: address):
	...
	self.voters[msg.sender].voted = True	# Mark weight as having benn assigned.
	self.voters[msg.sender].delegate = to	# Send to sombody else
...
def vote(proposal: int128):
	assert not self.voters[msg.sender].voted # can't vote twice
	assert proposal < self.int128Proposals   # can only vote on legitimate proposals

	self.voters[msg.sender].voted = True	 # Mark as having voted
	self.voters[msg.sender].vote = proposal	 # record what was voted for.

	# transfer msg.sender's weight to proposal
	self.proposals[proposal].voteCount += self.voters[msg.sender].weight
	self.voters[msg.sender].weight = 0		#  Discard weight - so can not use again.

# Computes the winning proposal, the proposal with the max number of botes.
@view
@internal
def _winningProposal() -> int128:
	winning_vote_count: int128 = 0	# Assume 0 votes to start
	winning_proposal: int128 = 0	# Assume proposal at locaiton 0
	for i in range(4):
